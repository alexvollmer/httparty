...
describe HTTParty::Response do
  ...
  it "should allow hashes to be accessed with dot notation" do
    response = HTTParty::Response.new({'foo' => 'bar'}, "{foo:'bar'}", 200, 'OK')
    response.foo.should == 'bar'
  end

  it "should allow nested hashes to be accessed with dot notation" do
    response = HTTParty::Response.new({'foo' => {'bar' => 'baz'}}, "{foo: {bar:'baz'}}", 200, 'OK')
    response.foo.should == {'bar' => 'baz'}
    response.foo.bar.should == 'baz'
  end

  xit "should allow iteration of each member with dot notation" do
    response = HTTParty::Response.new([{:name => "FOO"},{:name => "BAR"}],
                                      "[{foo:'FOO'},{bar:'BAR'}]",
                                      200, "OK")
    response.map { |x| x.name }.should == %w(FOO BAR)
    # response.each { |x| puts x.name }
  end
